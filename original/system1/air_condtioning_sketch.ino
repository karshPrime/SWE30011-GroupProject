...
void check_temp( void )
{
    if ( millis() - lastTimeTempRead > tempReadInterval )
    {
        ...
        // Lock the temperature control if within the interval, stops motor turning on an off
        // repeatedly untill the temperature stabilises below specified range.
...
// Toggles whether motor override is on or off, I.E the motor override light bulb and whether the
// motor is on itself
...
// Checks the motor override button to see if has been clicked and calls toggle_motor_override if it
// has been
void check_override_button( void )
{
    // Lock the checking behind a debounce duration to allow the button to stabilise back to LOW
    // state
